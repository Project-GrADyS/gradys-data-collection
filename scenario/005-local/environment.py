        ...
        def observe(self):
            ...
            if drone_count > 0:
                ...
                drone_observation[:drone_count] = (
                                                          own_position - drone_positions[
                                                      closest_drones] + self.scenario_size) / (self.scenario_size * 2)

            if sensor_count > 0:
                _, closest_sensors = sensor_kdtree.query(own_position, sensor_count)
                sensor_observation[:sensor_count] = (
                                                            own_position - sensor_positions[
                                                        closest_sensors] + self.scenario_size) / (
                                                                self.scenario_size * 2)
        ...
        def handle_packet(self, message: str) -> None:
            ...
            if message_type == 'S':
                ...
                for other_sensor in drone_message['known_sensors_age'].keys():
                    if drone_message['known_sensors_age'][other_sensor] > self.known_sensors_age.get(int(other_sensor),
                                                                                                     -1):
    ...
    def reset(self, seed=None, options=None):
        ...
        for _ in range(self.num_sensors):
            ...
            node_id = builder.add_node(
                create_sensor_protocol(self.min_sensor_priority, self.max_sensor_priority, self.agent_node_ids),
                (x_pos, y_pos, 0))
            self.sensor_node_ids.append(node_id)
            sensor_positions[node_id] = (x_pos, y_pos, 0)

        for _ in range(self.num_drones):
            protocol = create_drone_protocol(self.speed_action, self.algorithm_iteration_interval,
                                             self.state_num_closest_drones,
