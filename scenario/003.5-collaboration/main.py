...
def main():
    ...
    if args.use_priority:
        sampler = PrioritizedSampler(args.buffer_size, alpha=args.priority_alpha, beta=args.priority_beta)
    else:
        sampler = None
    ...
    all_agent_obs = np.stack([obs[agent] for agent in env.agents], dtype=np.float32)
    terminated = False
    for global_step in range(args.total_timesteps):
        ...
        if args.use_heuristics:
            ...
            all_agent_actions = np.stack([actions[agent] for agent in env.agents], dtype=np.float32)
        else:
        ...
        if args.use_heuristics:
            ...
            all_agent_next_obs = np.stack([next_obs[agent] for agent in env.agents], dtype=np.float32)
            ...
            data['priority'] = qf1_loss.expand(args.batch_size)
